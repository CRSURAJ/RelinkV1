...
mkdir -p ~/tb-gateway
sudo tee ~/tb-gateway/docker-compose.yml >/dev/null <<'EOF'
version: '3.8'
services:
...
#        - "50000:50000/udp" # Socket connector with type UDP
    ...
    volumes:
      ...
      # share the host’s mmcli and its libs (paths are distro-specific)
      - /usr/bin/mmcli:/usr/bin/mmcli:ro
...
TS_AUTHKEY=
TS_ADVERTISE_ROUTES=192.168.0.0/24
TS_EXTRA_ARGS=--ssh

TB_HOST=your host
TB_PORT=1883
TB_ACCESS_TOKEN=your access token
EOF
